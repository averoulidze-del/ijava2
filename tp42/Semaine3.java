class Semaine3 extends Program {
    ...
    String set(String image, int line, int column, String color) {
        int imageSize = size(image);
        int idxStart = line * imageSize * 9 + column * 9;
        return substring(image, 0, idxStart) + color + substring(image, idxStart + 9, length(image));
    }
    void trace() {
        if (penDown) {
    ...
    boolean go(int line, int column) {
        ...
        if (!inside(imageSize, line, column)) return false;
        lineTurtle = line;
        columnTurtle = column;
        trace();
        return true;
    ...
    boolean move(int[] direction) {
        return go(lineTurtle + direction[0], columnTurtle + direction[1]);
    }
