class ConditionsSurChaines extends Program {
    ...
    void algorithm(){
        ...
        boolean condAEgalB = length(a)>0 && equals(a,b);//À MODIFIER
        // Condition la première lettre de b précède la première lettre de a d’après l’ordre du dictionnaire
        boolean condBprecedeA = length(a)>0 && charAt(b, 0) < charAt(a, 0);//À MODIFIER
        // (Optionnel) Condition c est un prefixe de a
        boolean condCprefixeDeA = length(a)>0 && equals(c, substring(a, 0, length(a)/2)) ;//À MODIFIER
